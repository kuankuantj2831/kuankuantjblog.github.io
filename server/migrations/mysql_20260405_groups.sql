...
-- 增加帖子点赞数
CREATE PROCEDURE IF NOT EXISTS increment_post_likes(IN post_id CHAR(36))
BEGIN
    UPDATE group_posts SET like_count = like_count + 1 WHERE id = post_id;
END //

-- 减少帖子点赞数
CREATE PROCEDURE IF NOT EXISTS decrement_post_likes(IN post_id CHAR(36))
BEGIN
    UPDATE group_posts SET like_count = GREATEST(like_count - 1, 0) WHERE id = post_id;
END //

-- 增加帖子评论数
CREATE PROCEDURE IF NOT EXISTS increment_post_comments(IN post_id CHAR(36))
BEGIN
    UPDATE group_posts SET comment_count = comment_count + 1 WHERE id = post_id;
END //

-- 更新圈子成员数
CREATE PROCEDURE IF NOT EXISTS update_group_member_count(IN group_id CHAR(36))
BEGIN
    UPDATE groups 
    SET member_count = (SELECT COUNT(*) FROM group_members WHERE group_id = group_id),
        last_activity_at = NOW()
    WHERE id = group_id;
END //

-- 更新圈子帖子数
CREATE PROCEDURE IF NOT EXISTS update_group_post_count(IN group_id CHAR(36))
BEGIN
    UPDATE groups 
    SET post_count = (SELECT COUNT(*) FROM group_posts WHERE group_id = group_id),
        last_activity_at = NOW()
    WHERE id = group_id;
END //
