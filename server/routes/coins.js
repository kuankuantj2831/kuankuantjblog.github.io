...
// 硬币奖励配置
const COIN_REWARDS = {
    checkin_base: 2,        // 签到基础奖励
    checkin_streak_bonus: 1, // 连续签到每天额外奖励（最多+5）
    publish_article: 5,     // 发布文章奖励
    article_liked: 1,       // 文章被点赞奖励
    comment_article: 1,     // 评论文章奖励
    max_streak_bonus: 5     // 连续签到最大额外奖励
};
...
router.get('/balance', verifyToken, async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            return res.json({ balance: 0, total_earned: 0, total_spent: 0, last_checkin: null, checkin_streak: 0 });
        }
...
router.post('/checkin', verifyToken, async (req, res) => {
    try {
        ...
        // 计算奖励
        const streakBonus = Math.min(streak - 1, COIN_REWARDS.max_streak_bonus);
        const reward = COIN_REWARDS.checkin_base + streakBonus;
