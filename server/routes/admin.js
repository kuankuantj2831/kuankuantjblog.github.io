...
router.post('/login', (req, res) => {
    ...
    if (username === ADMIN_USERNAME && bcrypt.compareSync(password, ADMIN_PASSWORD_HASH)) {
        ...
        const token = jwt.sign({ role: 'admin_standalone' }, getJwtSecret(), { expiresIn: '24h' });
...
router.get('/users', requireAdmin, async (req, res) => {
    try {
        const [users] = await pool.query('SELECT id, username, email, phone, role, created_at, is_2fa_enabled FROM users ORDER BY created_at DESC');
        res.json(users);
    } catch (error) {
        console.error(error);
...
// PUT /admin/users/:id/2fa - 管理员禁用用户 2FA
router.put('/users/:id/2fa', requireAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const { enabled } = req.body;

        // 检查用户是否存在
        const [userRows] = await pool.query('SELECT id, username, is_2fa_enabled FROM users WHERE id = ?', [id]);
        if (userRows.length === 0) {
            return res.status(404).json({ message: '用户不存在' });
        }

        const user = userRows[0];
        const newStatus = enabled === true ? 1 : 0;

        // 更新 2FA 状态
        await pool.query('UPDATE users SET is_2fa_enabled = ? WHERE id = ?', [newStatus, id]);

        console.log(`Admin: 用户 ${user.username} (ID: ${id}) 的 2FA 状态已设置为 ${newStatus}`);

        res.json({
            message: `用户 ${user.username} 的二次验证已${newStatus ? '启用' : '禁用'}`,
            userId: parseInt(id),
            username: user.username,
            is_2fa_enabled: newStatus === 1
        });
    } catch (error) {
